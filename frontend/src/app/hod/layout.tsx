...
function hasHodAccess() {
  ...
  try {
    ...
    return isAllowedStaffEmail(email) && ["hod", "administrator"].includes(role);
  } catch {
    return false;
  }
}
...
export default function HodLayout({ children }: { children: ReactNode }) {
  ...
  const canAccess = hasHodAccess();

  useEffect(() => {
    if (!canAccess) router.push("/login");
  }, [canAccess, router]);
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-[#F5F7FA]">
      <div className="w-full shrink-0 border-b border-[#D5D4D4] bg-white">
        <HodHeader onMenuToggle={() => setSidebarOpen((prev) => !prev)} />
      </div>

      <div className={`relative flex min-h-0 flex-1 overflow-hidden lg:grid ${sidebarCollapsed ? "lg:grid-cols-[4.5rem_minmax(0,1fr)]" : "lg:grid-cols-[14rem_minmax(0,1fr)]"}`}>
        <aside className="relative hidden border-r border-black/10 bg-white lg:block">
          <HodSidebar collapsed={sidebarCollapsed} />
          <button
            type="button"
          ...
          >
            {sidebarCollapsed ? <FiChevronRight className="text-[18px]" /> : <FiChevronLeft className="text-[18px]" />}
          </button>
        </aside>

        <div className={`fixed inset-0 z-40 transition-opacity duration-200 lg:hidden ${sidebarOpen ? "opacity-100" : "pointer-events-none opacity-0"}`}>
          <button type="button" aria-label="Close sidebar" onClick={() => setSidebarOpen(false)} className="absolute inset-0 bg-black/30" />
          <aside className={`absolute left-0 top-0 h-full w-[min(84vw,320px)] border-r border-[#E2E8F0] bg-white shadow-xl transform transition-transform duration-200 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
            <HodSidebar collapsed={false} onNavigate={() => setSidebarOpen(false)} />
          </aside>
        </div>
