...
import GeneralOfficeSidebar from "../../components/general-office/GeneralOfficeSidebar";
import GeneralOfficeHeader from "../../components/general-office/GeneralOfficeHeader";

const ALLOWED_DOMAINS = ["@mitsgwl.ac.in", "@mitsgwalior.ac.in"];

function isAllowedGeneralOfficeAccess(email = "") {
  const normalizedEmail = String(email).toLowerCase();
  return ALLOWED_DOMAINS.some((domain) => normalizedEmail.endsWith(domain));
}

function canAccessGeneralOfficeFromStorage() {
  ...
  try {
    ...
    // Check if user has general office role or admin role
    const hasValidRole = ["generaloffice", "administrator"].some(
      (r) => role === r
    );
    
    return isAllowedGeneralOfficeAccess(email) && hasValidRole;
  } catch {
    return false;
  }
}

export default function GeneralOfficeLayout({ children }: { children: ReactNode }) {
  const router = useRouter();
  const pathname = usePathname();
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false);

  const hasGeneralOfficeAccess = canAccessGeneralOfficeFromStorage();

  useEffect(() => {
    if (!hasGeneralOfficeAccess) {
      router.push("/login");
    }
  }, [hasGeneralOfficeAccess, router]);
  ...
  return (
    <div className="h-screen bg-[#F5F7FA] flex flex-col overflow-hidden">
      ...
      >
        ...
        >
          <GeneralOfficeSidebar collapsed={sidebarCollapsed} />
        ...
        >
          ...
          >
            <GeneralOfficeSidebar collapsed={false} onNavigate={() => setSidebarOpen(false)} />
          </aside>
        </div>

        <main className="min-w-0 overflow-y-auto bg-[#F5F7FA] p-3 sm:p-4 lg:p-4">
